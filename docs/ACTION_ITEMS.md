...
- ☑ **TID-2 → dictionary/LookupPanel — DONE 2026-06-13:** footer popup tra từ giờ có **💾 Lưu nhanh** (→ SRS/Kho từ) + **📁 Bộ thẻ** (→ picker `openAddToDeckForWord` deck `hsk_decks_v3`, đóng panel trước khi mở picker) + link Từ điển. `lookup-panel.js?v=1.2` (`_addToDeck`). Gộp cùng việc "gom tap-gloss toàn app" bên dưới.
- ☑ **TID-3 → C1 v2 (tab Luyện dịch) — DONE 2026-06-13** (commit `a95d1c5` branch `c1-lesson-practice`, chờ merge): mode **"câu mẫu che từ"** — đáp án hiện chip che (`黑**` lộ chữ đầu + độ dài), click/Ctrl+Space mở dần từng từ, "Hiện tất cả" = đầu hàng. Deterministic 0 AI. (Validate luôn hướng guided-stepper §9: TID cũng stepper 5 bước.)
- ☑ **TID-4 → C7 — DONE 2026-06-13** (commit `b29fb16` main): khung copy **"Trước → Sau khi học" per-skill** (4 nhóm từ vựng/nghe-nói/đọc/thi) đã build vào `pages/pricing.html` (`.pricing-baprep`) — mô tả thay đổi năng lực, compliance-safe (note disclaimer "kết quả phụ thuộc quá trình học"). (Chưa nhân bản sang free-vs-pro — optional, copy đã có ở pricing.)
...
- 🔶 **C7 · Conversion home khách + pricing — CORE DONE 2026-06-13** (commit `b29fb16` main): trust strip khách (`#homeGuestStrip` 3 bullet/4 chip/CTA) + testimonial THẬT (component tái dùng `Testimonials`/`PublicStats` trong `community.js`, đọc RPC Tri Ân v24 + `public_stats` v26, ẩn khi rỗng, ngưỡng `MIN_USERS=100`) + pricing redesign (Trước→Sau, "Yên tâm khi mua" refund khớp terms §5.1, testimonial block, footer CTA). **SQL v24 + v26 ĐÃ chạy prod 2026-06-13.** Compliance B3 pass (0 fake số/urgency). 📐 spec `docs/plans/cs-hskclub/07-c7-landing-pricing-conversion.md`.
  - ☐ **B2 — CHỜ USER QUYẾT (chưa build):** (a) marketing welcome-gift AI credit rõ hơn trên pricing [0 backend] **vs** (b) trial Pro time-boxed 3 ngày [cần cột `trial_until` + chống farm, rủi ro abuse]. Spec mặc định (a); (b) chỉ khi user duyệt riêng.
  - ☐ **Price-lock khi gia hạn ("giữ giá đã mua") — CHỜ USER QUYẾT (chưa build):** chỉ đưa lên copy nếu user CAM KẾT vận hành được (giữ giá cũ khi renew). Chưa xác nhận → KHÔNG hứa trên trang.
...
- ☑ **DONE 2026-06-13 — v2 GUIDED refactor** (commit `a95d1c5` trên branch, spec §9 + TID-3): stepper 8 bước mặc định (0 Bối cảnh: context+scene+chips+banner "học truyện trước" dismiss-nhớ) · tab→sheet bước (jump tự do, số câu CHỈ trong sheet; desktop tab bar wrap không tràn, bỏ badge số, ✓ done) · resume `_lastStep`/`_lastIdx` ("Tiếp tục Bước k" + nhãn sidebar; resume vào màn result → "đã hoàn thành" không phải 0%) · mobile header 1 hàng (‹ title ⋯, menu ⋯ = Nghe/Từ vựng/DS bài) + stepper dots · Hội thoại 2 toggle pinyin/nghĩa · Tổng hợp cap 12 + "Làm thêm" · **TID-3 mode "che từ"** Luyện dịch (chip lộ chữ đầu+độ dài, click/Ctrl+Space mở dần giữ nguyên chữ đang gõ, "Hiện tất cả"=đầu hàng→SRS, gõ+Enter nộp normalize). Bump: lesson-practice.js/css→2.0, icons.js→1.4 (+`more-h`).
  - **Verify §9.4+§8** (app thật, worktree :5191): flow nguyên bài 1 end-to-end chỉ bằng "Bước tiếp theo" · resume đúng bước đúng câu sau reload · jump sheet giữ state bước dở · bài 115 thiếu order → 7 bước đánh số liền mạch (scan 154 bài: 0 lỗi, chỉ 115/118/120/121 thiếu order) · mobile 375 header+stepper 1 hàng không tràn ngang · sai → SRS (fill 0→1, dictation →5, đầu hàng →6) · dictation autogen URL R2 đúng pattern · gate 115 free→gated · Enter/1-4/Ctrl+Space OK · mix deterministic sau reload · light+dark · console 0 error.
- ☑ **DONE 2026-06-13 — Bước Hội thoại "chất truyện nhẹ"** (commit `a969be7`, spec §⚖️ DECISION 2026-06-12, phương án A): scene banner mỏng đầu màn (tái dùng `bg` step đầu qua `_sceneBgUrl()`) · avatar/expression theo TỪNG DÒNG (Mai = sprite expression qua `Course.exerciseAPI.maiImg` mới; speaker khác = ảnh cast tĩnh; fallback emoji) · narrator dòng dẫn nhạt+italic không avatar (`lp-doc-hanzi--narr`) · "Nghe toàn bộ" highlight karaoke dòng đang phát + scroll theo (`_setPlayingLine`, queue mang index gốc, clear khi stop/playDoc đơn). GIỮ: 1 màn transcript, 2 toggle pinyin/nghĩa. **Phân vai bất biến: /course=HỌC visual-novel · /lesson-practice=ÔN drill.** Verify :5191 — scene+14 dòng id, Mai sprite `curious.webp`, bài 13 narrator italic không avatar, highlight chạy+cleanup, mobile 375 + light/dark không tràn, console 0 error. Bump: course.js→3.9, lesson-practice.js/css→2.1.
- ☑ **MERGED → `main` 2026-06-13** (3 merge commit: `8fd4b54` v1+v2 · `0f2eb6e` Hội thoại chất truyện; branch `c1-lesson-practice` giữ lại). **CHƯA push GitHub** (local). docs/ local-only.
...
- ☑ **Q1 — Wire metric — DONE 2026-06-13** (chặn các quest mới): metric mới `reader_done`/`pinyin_drill`/`shadow_practiced`/`shadow_graded`/`hskk_graded`/`writing_graded`/`tutor_msg`/`vocab_saved`/`community_visit` + metric treo còn dùng (`dict_opened`/`correct_streak`/`perfect_mcq`/`typing_answers`/`hard_studied`/`new_cards`/`deck_completed`) + tổng hợp `skills_today`. **Nhớ thêm field vào 2 block reset `qd.metrics` (quests.js:216 & :235) — thiếu = `incrementMetric` no-op.**
- ☑ **Q2 — Data quest — DONE 2026-06-13:** append vào `DAILY_TIER_*` / `WEEKLY_QUESTS` / `QUEST_CHAINS` (§6-8 doc). Chain mới: `speaker`/`reader`/`toolsmith` + onboarding `rookie`.
- ☑ **Q3 — UI còn thiếu — DONE 2026-06-13:** `QUEST_CHAINS` đã có data nhưng `renderQuestPage` **chỉ render daily/weekly** → chain chưa từng hiện. Thêm tab "Hành trình" + achievement ở `pages/quests.html`. Onboarding `rookie` one-time (lưu `qd.chains.rookie`, ẩn khi xong).
...
- ☑ **Phần B — Trang Tri Ân: CODE pushed `main` `6b08f32`**, ⚠️ **CHỜ chạy SQL prod** mới live:
  ...
  - Admin curation: `js/admin/feedback.js` — nút "Duyệt công khai" (`admin_set_feedback_approved`, disable nếu user chưa consent) + "Đặt làm MVP" (`admin_set_feedback_featured`, chỉ 1 MVP). Card hub đổi locked → link `/tri-an`.
  - **Verified in-browser:** light + dark + mobile 375px, 0 lỗi console, empty-state khi RPC missing OK, checkbox consent render OK. Bump `?v=`: home.css 2.6 · feedback.js 2.8 · admin/feedback.js 1.2.
  - **Việc của bạn:** chạy `v24` trên Supabase SQL Editor → ghi "DEPLOYED prod" vào đầu file SQL. Sau đó admin duyệt vài góp ý có consent → wall hiện.
...
- ☑ DONE 2026-06-13 — **Split HSK3 lvl5-9 on-demand**: index.html chỉ load L1-4 (~0.7MB); L5-9 (~2.3MB) tách thành bundle `hsk3_rest` trong `data-loader.js`. Prefetch nền sau boot (`app.js` `_prefetchVocabRest`, rIC timeout 1500) → re-render lưới cấp/bản đồ/stats khi data về (`hsk3-vocab-ready` event). Route vocab (dictionary/quiz/learn/vault/games/my-vocab/mock-exam/text-analyzer/topics/ban-do-hsk/profile…) gate qua `_pageBundles['hsk3_rest']` = lưới an toàn mạng chậm. Self-heal sẵn: lookup-panel `_wordSet` rebuild khi vocab tăng; home grid `displayCount` fallback LEVEL_INFO_V3.count nên không hiện "0 từ". **Verified preview:** initial HTML chỉ L1-4 · prefetch → đủ 11.456 từ (L1-9) · tra từ 唉/L5 OK · 0 console error.
- ☐ **#4 Reader:** đẩy nốt ~20 bài HSK1-3 đạt ~50 + duyệt batch chờ → bật SEO `/doc-truyen` (xem follow-up A1 session 2026-06-07).
- ☑ DONE 2026-06-13 — **#5 Service worker** (PWA bước 1): `sw.js` (root, scope /) + đăng ký ở `app.js` `_registerServiceWorker` (`updateViaCache:'none'`, chỉ https/localhost). HTML/navigation = **network-first** (online luôn lấy markup mới → ?v= mới; offline fallback shell) → hard-reload nhận bản mới + offline mở shell. Asset versioned ?v= = **cache-first** (bump ?v= = URL mới = fetch). Cross-origin (Supabase/CDN) passthrough không cache. Kill-switch: `window.__killSW()` runtime + đoạn sw.js "tự sát" (comment đầu file) cho diện rộng. skipWaiting qua message + clients.claim + dọn cache cũ ở activate; controllerchange auto-reload 1 lần (bỏ qua lần cài đầu). **Verified preview:** SW activated+controlling · cache `hanzigenz-v1` có shell `/`+`/index.html` (offline OK) · không reload loop · 0 error.
- 🔶 **Bật JWT verify lại cho `create-payment` — CODE DONE 2026-06-13, ⚠️ CHỜ USER DEPLOY**: function verify Bearer JWT in-code (`auth.getUser(token)`), lấy `userId/email` từ token — **KHÔNG còn tin `body.userId`** (chống spoof + spam `payment_orders`). Thêm env `SUPABASE_ANON_KEY` (Supabase auto-inject, không cần set tay). Client `pricing.js?v=2.7`: token giờ BẮT BUỘC (thiếu → báo đăng nhập lại), VẪN gửi `userId` trong body để tương thích ngược trong cửa sổ deploy. **`supabase/` gitignored → edit nằm local, KHÔNG commit.** Việc user: (1) `supabase functions deploy create-payment` (client vẫn gửi userId nên deploy client trước function không vỡ); (2) test 1 user thật login → mua → tới QR PayOS; (3) curl gọi function KHÔNG kèm Authorization → kỳ vọng 401. Tuỳ chọn lớp 2: deploy KHÔNG `--no-verify-jwt` (gateway verify) — in-code đã đủ.
- ☑ **Self-host `supabase.js`+`hanzi-writer` vào `vendor/` — DONE 2026-06-13**: `vendor/hanzi-writer-3.5.min.js` (v3.5.0) + `vendor/supabase-js-2.umd.js` (v2.108.1, pin từ floating `@2`); `index.html` trỏ vendor (`?v=1.0`, load TRƯỚC `js/supabase.js`), bỏ 2 `<script>` jsdelivr. **Verified preview:** `window.supabase.createClient`+`SB`+`window.HanziWriter` đều sẵn, 0 console error, 0 failed request. SW cache-first same-origin nên vendor được cache (offline-ready). **Còn lại (non-critical, ngoài phạm vi):** `hanzi-writer-data@2.0` (JSON nét chữ per-char, 6 chỗ) + `chart.js` (admin-only) vẫn jsdelivr — degrade an toàn (mất animation nét/chart, KHÔNG chặn login).
...
- 🟢 **Gom tap-to-gloss TOÀN APP — phần lớn DONE 2026-06-13:** `LookupPanel` (`lookup-panel.js?v=1.2`) giờ là component DUY NHẤT cho tap-gloss đọc nội dung: hỗ trợ **tap + mobile** (bỏ chặn `_isDesktop` ở `open`/`_onClick`; <1024px render **bottom-sheet** `lookup-panel.css?v=1.1` `.lp-panel--sheet`, tap-ngoài để đóng) + **tách-từ longest-match** cho text tự do (`_segmentAtPoint` snap caret + match ≤4 chữ qua `_wordSet`). **Bỏ 2 popup trùng:** `speaking.js?v=1.7` (line giờ plain text + `data-lookup="true"`, xài global panel) + `reader.js?v=1.1` (giữ resolve gloss curated per-câu, render qua `LookupPanel.open(...,{word})` → 1 UI, không mất nghĩa-trong-ngữ-cảnh). Hover desktop GIỮ NGUYÊN (vẫn `_isDesktop`-gated). Verify Playwright desktop 1280 + mobile 375, 0 console error. **Còn lại (cố ý CHƯA gộp — đã có popup riêng tốt hơn, gộp = regression/double-fire):** `course.js` Mai (`cs-vn-wordpop` có highlight từ-trọng-tâm + autospeak) · `text-analyzer.js` (`taPopup` có highlight-theo-cấp). `mock-exam` KHÔNG đánh dấu (tra giữa bài thi = gian lận). grammar/reading vốn đã dùng global panel → giờ chạy luôn trên mobile.
...
- ☑ **Regen ví dụ từ điển DONE (verify 2026-06-07):** toàn bộ 11.456 từ L1–L9 đã 100% có ví dụ (`gen-dict-examples.js stats` = 0 need AI). Ghi chú "~1.001 từ trống" đã stale — refill từ session trước. · ☐ Còn: nâng grammarNotes HSK 4 thành ngữ pháp thật (xem BUG/prompt tổng hợp).
...
- ☑ **Mở rộng kho đề HSKK DONE (2026-06-07, commit `63f6f4d`):** pool đã đủ rộng từ các session trước (Sơ cấp part1 50/part2 40; Trung cấp part1 40/part2 25) — chỉ còn part3 mỏng → top-up part3 15→18 cả 2 cấp. Ghi chú "part1 20/part2 14" đã stale. `so-cap.js?v=1.2` · `zhong-ji.js?v=1.3`.
