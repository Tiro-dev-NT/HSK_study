...
| Sync error UI feedback | ❌ Silent fail | [`js/sync.js`](../js/sync.js):32,54,74,89,109 — push fail chỉ `console.error`, user không biết mất data |
| Supabase query không có `.limit()` | ⚠️ Unbounded | [`js/sync.js`](../js/sync.js):135,165 + [`js/quests.js`](../js/quests.js):219 — sẽ timeout khi user 5k+ SRS / 2k+ ledger rows |
| Partial sync batch failure | ⚠️ Split-brain | [`js/sync.js`](../js/sync.js):52-55 — batch SRS 500/lần, batch N+1 fail → local-remote lệch |
| SKU param validation | ⚠️ Client-only | [`js/pricing.js`](../js/pricing.js):118 — `sku` từ URL truyền thẳng Edge Function, cần whitelist server |

**Audit chi tiết:** xem [SCALING_PLAN.md](SCALING_PLAN.md) section 2 (storage migration) + section 5 (AI cost runaway).
...
- [SCALING_PLAN.md](SCALING_PLAN.md) — migration storage/DB/hosting + chi phí từng giai đoạn
