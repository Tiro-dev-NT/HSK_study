...
var BossBattle = {
  ...
  _answer: function(btn, isCorrect) {
    ...
    if (isCorrect) {
      ...
      BossBattle.bossHP = Math.max(0, BossBattle.bossHP - cfg.playerDmg);
      BossBattle._shakeEl('.boss-enemy');
    } else {
      ...
      var dmg = cfg.bossDmg;
      // Boss skill: every 3 turns, damage x2
      if (cfg.skill && BossBattle.turn % 3 === 0) {
        dmg *= 2;
        var msg = document.getElementById('bossSkillMsg');
        if (msg) { msg.textContent = '💥 ' + cfg.name + ' dùng Tuyệt chiêu! Sát thương x2!'; msg.style.display = ''; }
        var t = setTimeout(function() { if (msg) msg.style.display = 'none'; }, 2000);
        BossBattle.timers.push(t);
      }
      BossBattle.playerHP = Math.max(0, BossBattle.playerHP - dmg);
      BossBattle._shakeEl('.boss-player');
    }
  ...
  _updateHP: function() {
    ...
    if (pFill) pFill.style.width = Math.max(0, BossBattle.playerHP / BossBattle.playerMaxHP * 100) + '%';
    if (eFill) eFill.style.width = Math.max(0, BossBattle.bossHP / BossBattle.bossMaxHP * 100) + '%';
