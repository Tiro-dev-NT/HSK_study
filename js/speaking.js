var Speaking = (function () {
  ...
  // Tap-to-gloss is handled by the shared global LookupPanel (js/lookup-panel.js):
  // the practice line container carries data-lookup="true" so taps/hover resolve
  // words via longest-match against the dictionary. No per-page popup here.
  function _closeLookup() {
    if (typeof LookupPanel !== 'undefined' && LookupPanel.close) LookupPanel.close();
  }
  ...
  function _renderLine() {
    ...
    var hzEl = $('spLineHanzi');
    hzEl.setAttribute('data-lookup', 'true');
    hzEl.textContent = line.h;
